...
export const RemindersTab: React.FC<RemindersTabProps> = ({
  ...
  const getCategoryBadge = (category: string) => {
    switch (category) {
      case 'vaccine':
        return { label: 'Вакцинация', color: 'bg-teal-500/20 text-teal-300 border-teal-500/30' };
      case 'parasite':
        return { label: 'От паразитов', color: 'bg-amber-500/20 text-amber-300 border-amber-500/30' };
      case 'grooming':
        return { label: 'Груминг', color: 'bg-purple-500/20 text-purple-300 border-purple-500/30' };
      case 'medicine':
        return { label: 'Лекарство', color: 'bg-rose-500/20 text-rose-300 border-rose-500/30' };
      case 'vet':
        return { label: 'Визит к ветврачу', color: 'bg-blue-500/20 text-blue-300 border-blue-500/30' };
      default:
        return { label: 'Забота', color: 'bg-slate-700 text-slate-300 border-slate-600' };
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-300 text-left">
      ...
      <div className="reminders-tab flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-cyan-950/60 via-slate-900 to-teal-950/60 p-6 rounded-3xl border border-cyan-500/30">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-400 hover:to-teal-400 text-slate-950 font-extrabold text-sm shadow-lg shadow-cyan-500/25 transition cursor-pointer"
        >
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-900/80 p-3 rounded-2xl border border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-slate-950 text-xs text-slate-200 font-semibold rounded-xl border border-slate-700 px-3 py-1.5 focus:border-cyan-400 focus:outline-none"
          >
            ...
            <div
              ...
              className={`p-5 rounded-2xl border transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
                rem.isCompleted
                  ...
                  : 'bg-slate-900 border-slate-800 hover:border-cyan-500/40 shadow-lg'
              }`}
            >
              <div className="flex items-start gap-4">
                <button
                  ...
                  className={`mt-0.5 w-6 h-6 rounded-lg flex items-center justify-center transition-colors cursor-pointer ${
                    rem.isCompleted
                      ...
                      : 'border-2 border-slate-600 hover:border-cyan-400 text-transparent'
                  }`}
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className={`text-base font-bold ${rem.isCompleted ? 'line-through text-slate-400' : 'text-white'}`}>
                      {rem.title}
                    </span>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-cyan-500/40 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleCreateReminder} className="space-y-4 text-left">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white focus:border-cyan-400 focus:outline-none"
                />
              </div>
              ...
              <div className="pt-4 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-6 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-extrabold text-xs shadow-md cursor-pointer"
                >
                  Сохранить
                </button>
              </div>
