...
import { LostAlert, OwnerProfile, Pet } from '../types';
...
export const SOSAlertModal: React.FC<SOSAlertModalProps> = ({
  ...
  const [phone, setPhone] = useState(owner.phone || pets[0]?.emergencyContacts[0]?.phone || '');
  ...
  if (sent) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl">
        <div className="max-w-md w-full rounded-3xl bg-slate-900 border border-rose-500/40 p-6 text-white">
          <h3 className="text-xl font-extrabold mb-2">Поиск включён</h3>
          <p className="text-slate-300 text-sm mb-5">Сигнал сохранён в разделе «Потеряшка SOS». Нашедший по QR сможет передать местоположение.</p>
          <button type="button" onClick={() => { setSent(false); onClose(); }} className="px-5 py-3 rounded-xl bg-rose-600 font-extrabold">Закрыть</button>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newAlert: LostAlert = {
      ...
      ownerName: owner.name || currentPet.emergencyContacts[0]?.name || 'Владелец',
