...
export const AddPetModal: React.FC<AddPetModalProps> = ({
  ...
  function ageFromBirth(value: string) {
    ...
    if (m < 0) { y -= 1; m += 12; }
    if (y > 0) return `${y} ${y === 1 ? 'год' : y < 5 ? 'года' : 'лет'}`;
    if (m > 0) return `${m} мес.`;
    return 'младше месяца';
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newPet: Pet = {
      ...
      species,
      breed,
      ageText: ageFromBirth(birthDate),
      birthDate,
      gender,
      photoUrl,
      zmId: newZmId,
      ...
      primaryVet: '',
      emergencyContacts: [
        { name: owner?.name || 'Владелец', phone: owner?.phone || '', relation: 'Основной владелец' },
      ],
